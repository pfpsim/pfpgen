...
from ..semantic.hlir import Service
from .AstNode import AstNode

class ServiceDeclaration(AstNode):
    def __init__(self, names, lineno):
        super(ServiceDeclaration, self).__init__(lineno)
        self.names = names
    ...
    def _analyze_pass_1(self, symbol_table):
        for name in self.names:
            symbol_table[name.name] = Service(name.name, name.config_path)
